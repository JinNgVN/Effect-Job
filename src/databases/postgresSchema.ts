...
export const postgresMigration = (
    ...
    return {
        up: [
            ...
            `  CONSTRAINT ${indexName(options, "priority_check")} CHECK (priority >= 0 AND priority <= 9),`,
            `  CONSTRAINT ${indexName(options, "attempt_check")} CHECK (attempt >= 0 AND max_attempts >= 1)`,
            `);`,
